...
Na podstawie sprecyzowanych powy¿ej zasad gry i opisu dzia³ania systemu opracowano przypadki u¿ycia systemu. Diagramy przypadków u¿ycia przedstawiono na rysunkach \ref{fig:usecase_1} oraz \ref{fig:usecase_2}.

\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{rys04/usecase1.jpg}
\caption{Diagram przypadków u¿ycia dla aplikacji mobilnej}
\label{fig:usecase_1}
\end{figure}
...
\item	U¿ytkownik klika przycisk "`Wybór zdjêcia"'.
		...
		\item	U¿ytkownik ustawia przyciêcie i zatwierdza wybór.
...
\begin{enumerate}
	...
	\item	Na serwerze nastêpuje sprawdzenie, czy wprowadzony kod jest poprawny oraz czy lokalizacje generowania i wprowadzenia kodu znajduj¹ siê w pobli¿u siebie.
	\item	Jeœli kod jest poprawny, zlecenie zostaje zatwierdzone. Aplikacja ponownie pobiera zmienione ju¿ dane o zleceniu.
	\item	Widok "`Szczegó³y zlecenia"' jest aktualizowany nowymi danymi.
\end{enumerate}
...
\item [Opis przebiegu interakcji]  \hfill
\begin{enumerate}
\item	W aplikacji wyœwietlony zostaje widok "`Generowanie kodu"'.
...
\item	Pola z informacjami ponownie staj¹ siê nieedytowalne.
...
\newpage
\section{Projekt interfejsu aplikacji mobilnej}

Projekt interfejsu aplikacji mobilnej zosta³ wykonany w oparciu o wskazówki \textit{Material design}\cite{design} opracowane przez Google. Projekt wykonany zosta³ w programie graficznym bez uwzglêdnienia dostêpnych technologii. Po implementacji niektóre elementy interfejsu mog¹ ró¿niæ siê nieznacznie od zaprojektowanych.

Po uruchomieniu aplikacji wyœwietlony zostaje ekran powitalny, z którego u¿ytkownik mo¿e przejœæ do logowania lub rejestracji do gry. Z poziomu ekranu powitalnego jest równie¿ mo¿liwoœæ wyœwietlenia zasad gry. Po klikniêcie przycisku "`Login"', wyœwietlony zostaje ekran "`Logowanie"' widoczny na rysunku \ref{fig:login}, a po klikniêciu przycisku "`Register"' ekran "`Rejestracja"' (rys. \ref{fig:register}).
\begin{figure}[h]
\centering
\includegraphics[width=4cm]{rys04/welcome}
\caption{Ekran startowy aplikacji, \textit{Ÿród³o obrazu:} \cite{boss}}
\label{fig:welcome}
\end{figure}

Na ekranie rejestracji formularz na informacje o u¿ytkowniku. Tekst wpisywany pole has³a nie jest wyœwietlany jawnie, lecz zastêpowany kropkami. Na prawo od pola has³a znajduje siê przycisk z ikon¹ oka, po jego naciœniêciu znaki has³a zostan¹ pokazane.

U góry ekranu znajduje siê miejsce na zdjêcie profilowe u¿ytkownika. Po klikniêciu wewn¹trz widocznej na rysunku \ref{fig:register} ramki u¿ytkownik zostanie przekierowany do odpowiedniej dla jego urz¹dzenia aplikacji galerii w celu wyboru zdjêcia. Po wyborze zdjêcia aplikacja poprosi u¿ytkownika o przyciêcie go do zadanych rozmiarów. Zdjêcia profilowe musz¹ byæ kwadratowe. Po zakoñczeniu procesu przycinania wybrane zdjêcie zostanie wyœwietlone wewn¹trz ramki.

Po klikniêciu przez uzytkownika w pole "`data urodzenia"' zamiast standardowej klawiatury wyœwietlone zostanie okno dialogowe z kalendarzem widoczne na rysunku \ref{fig:reg2}.
...
Widok "`Lista zleceñ"' wyœwietlany po zakoñczeniu zapisu lokalizacji podzielony jest na dwie jednakowe sekcje: prywatn¹ i publiczn¹. W ka¿dej z sekcji znajduje siê lista zleceñ danego typu. Elementy listy zawieraj¹ informacje o u¿ytkowniku bêd¹cym celem zlecenia oraz jego status. Po klikniêciu elementu listy otwarty zostaje widok "`Szczegó³y zlecenia"' widoczny na rysunku \ref{fig:details}. 
...
\caption{Zg³oszenie z³amania zasad}
\label{fig:det2}
\end{subfigure}
\caption{Ekran "`Szczegó³y zlecenia"'}
\label{fig:details}
\end{figure}


Po wykonaniu gestu przeci¹gniêcia w dó³ na ekranie szczegó³ów zlecenia wyœwietlona zostaje "`Mapa"' (rys. \ref{fig:map}), na którym znajduje siê mapa z naniesonymi znacznikami oznaczaj¹cymi znane lokalizacje szukanego gracza. Po klikniêciu na znacznik pokazana zostanie data i godzina zarejestrowania tej lokalizacji.
...
Boczne menu nawigacyjne pokazane na rysunku \ref{fig:drawer} przywo³ywane jest poprzez naciœniêcie przycisku nawygacyjnego lub przeci¹gniêcie w prawo na ekranie "`Lista zleceñ"'. Pozwala ono na prze³¹czanie pomiêdzy kolejnymi ekranami aplikacji. U do³u menu nawigacyjnego znajduj¹ siê równie¿ przyciski pozwalaj¹ce na kontakt z twórc¹ aplikacji oraz otwarcie strony aplikacji w sklepie Google Play.
...
Poni¿ej zaprezentowano diagram zwi¹zków encji w bazie danych prezentuj¹cy relacje pomiêdzy poszczególnymi tabelami (rys. \ref{fig:model_k}).
...
Kod generowany w aplikacji musi byæ przypisany do uzytkownika generuj¹cego go. U¿ytkownik mo¿e nie posiadaæ wygenerowanych kodów, lub posiadaæ dok³adnie jeden, jednak to ograniczenie nie zosta³o uzglêdnione w relacjach encji z za³o¿eniem, ¿e zostanie wprowadzone na etapie implementacji. 
...
Na rysunku \ref{fig:model_f} zaprezentowano model fizyczny bazy danych.
...
W celu uzyskania dostêpu do \texttt{/api/*} lub \texttt{/admin/*} wymagane jest przes³anie wraz z zapytaniem nag³ówka \texttt{X-Auth-Token} zawieraj¹cego informacje o uprawnieniach. Szczegó³y dotycz¹ce tego zabezpieczenia opisane zosta³y w sekcji \ref{zabezpieczenia}.

\subsection{Œcie¿ki dostêpu API}
Wybrane adresy obs³ugi zapytañ prezentuje poni¿sza lista. Struktury danych wymienione w sekcjach "`Parametry danych"' i "`Zwaracane wartoœci"' zosta³y przedstawione w kolejnym punkcie.
...
\newpage \item Pobranie konta u¿ytkownika
...
\newpage \item	Pobranie lokalizacji  u¿ytkownika
...
\newpage \item	Pobranie najbardziej aktualnej lokalizacji 
...
\newpage \item Zg³oszenie z³amania zasad
...
\newpage \item	Usuniêcie konta u¿ytkownika
...
\newpage	\item Pobranie listy u¿ytkowników
...
\newpage	\item Nadanie lub odebranie uprawnieñ administracyjnych 
...
Poni¿szy spis przedstawia struktury danych w formacie JSON przesy³ane podczas komunikacji pomiêdzy serwerem a aplikacjami klienckimi. W celu uproszczenia opisu zagnie¿dzone struktury  zosta³y zast¹pione ich nazw¹, np. \texttt{<User>}, a wartoœci numeryczne ich typem.
\begin{enumerate}
